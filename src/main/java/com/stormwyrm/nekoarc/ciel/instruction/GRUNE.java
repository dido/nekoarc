...
public class GRUNE implements CielInstruction {
    @Override
    public void invoke(Ciel c) {
        long rune = c.readLong();
        if (rune < 0 || rune > 0x10ffff)
            throw new NekoArcException("invalid Unicode rune in marshalled data");
        c.push(Rune.get((int) rune));
    }
}
